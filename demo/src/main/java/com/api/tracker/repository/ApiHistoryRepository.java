...
public interface ApiHistoryRepository extends JpaRepository<ApiHistory, Long> {
    ...
    @Query("SELECT a FROM ApiHistory a WHERE a.microserviceName = ?1 AND a.apiName = ?2 AND a.iterationCount < (SELECT MAX(b.iterationCount) FROM ApiHistory b WHERE b.microserviceName = a.microserviceName AND b.apiName = a.apiName) ORDER BY a.iterationCount DESC")
    List<ApiHistory> findPreviousDeliveries(String microserviceName, String apiName);

    @Query("SELECT a FROM ApiHistory a WHERE a.microserviceName = ?1 AND a.apiName = ?2 AND a.iterationCount = (SELECT MAX(b.iterationCount) FROM ApiHistory b WHERE b.microserviceName = a.microserviceName AND b.apiName = a.apiName)")
